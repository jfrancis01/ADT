...
public class PathMaxProbaility {
	...
	public double maxProbability(int n, int[][] edges, double[] succProb, int start, int end) {
		...
		PriorityQueue<Link> pq = new PriorityQueue<>((a, b) -> Double.compare(a.weight, b.weight));
		...
		pq.add(new Link(start, 1));
		while(!pq.isEmpty()) {
			Link link = pq.poll();
			int to = link.to;
			double weight = link.weight;
			if(to == end) return weight;
			if(!visited.add(to))continue;
			List<Link> children = graph.get(to);
			for(Link child : children) {
				pq.offer(new Link (child.to, child.weight * weight));
			}
	...
	public double maxProbability2(int n, int[][] edges, double[] succProb, int start, int end) {
		...
		HashMap<Integer, List<Link>> graph = new HashMap<Integer, List<Link>>();
		// add all the nodes to the distance map and set the distance of the starting
		// node to 0;
		for (int i = 0; i < n; i++) {
			if (start == i) {
				dist.put(i, 1.0);
